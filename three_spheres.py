...
tan = Material(vec([0.4, 0.4, 0.2]), k_s=0.3, p=90, k_m=0.5)
blue = Material(vec([0.2, 0.2, 0.5]), k_m=0.5)
gray = Material(vec([0.2, 0.2, 0.2]), k_m=0.4)

scene = Scene([
  Cube(vec([-0.7, 0, 0]), 1, blue),
  Sphere(vec([0.7, 0, 0]), 0.5, tan),
  Sphere(vec([-0.7, 0, 1.4]), 0.5, tan),
  Cube(vec([0, -40, 0]), 79, gray),
])
...
camera = Camera(vec([4, 1.2, 5]), target=vec(
